"""Run the frozen GWTC-5 confirmatory analysis.

The script validates the locked 18-event sample, loads paired compact posterior
arrays for the XPHM and XPNR analyses, computes the prespecified disagreement
metrics and split-sample reference, tests H1--H3, and writes tables, figures,
logs, and a provenance manifest under ``results/confirmatory_analysis``.

Inputs are expected beneath a GWTC-5 project root. The root may be supplied on
the command line, set with ``GWTC5_PROJECT_ROOT``, or discovered from the
current directory. Existing result files are protected unless ``--overwrite``
