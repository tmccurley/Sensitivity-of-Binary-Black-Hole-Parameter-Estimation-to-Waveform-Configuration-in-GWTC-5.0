"""Run post-confirmatory sensitivity checks added during manuscript revision.

The script reads the frozen confirmatory endpoints and parameter metrics, then
adds design-aware H1/H2 permutations, an H1 bootstrap interval and partial
correlation, alternative H3 parameterizations, practical screening summaries,
and a one-dimensional audit of the stored posterior arrays.

These outputs are exploratory supplements to the confirmatory analysis. The
caller must provide the GWTC-5 project root and a separate output directory.
"""
